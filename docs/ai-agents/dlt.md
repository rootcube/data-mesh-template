...
`source_dataset()` in `dlt_pipelines/utils/destination.py` is `SnowflakeSettings.from_env().schema_for_layer("src")`, so dlt follows exactly the rule dbt uses (`dbt_common.generate_schema_name`) and the source YAML repeats from the dbt target. A blank `SNOWFLAKE_SCHEMA` in `dev` resolves to the placeholder `DBT_SRC`, which nobody has, so the load fails instead of writing into the shared `_SRC`. The role that owns this layer in deployed environments is `RL_<PROJECT>_<ENV>__ING` ("used by ingestion tooling to load data into the SRC layer" in `terraform/config/roles/ingest.yaml`); in `dev` your engineer role inherits it.
...
- `[load] truncate_staging_dataset = true`: the `merge` staging tables in `_TMP` are emptied after each load (`LOAD__TRUNCATE_STAGING_DATASET` overrides it).
