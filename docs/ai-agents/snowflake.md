...
Platform administrators provision everything with Terraform from the YAML under `terraform/config/` ([Snowflake provisioning](../administration/snowflake-provisioning.md)); engineers never run Terraform. What one project becomes, per `terraform/main.tf`, `terraform/personal.tf`, `terraform/stages.tf` and the modules under `terraform/modules/snowflake/`:
...
| dlt load stage | internal stage `ST_DEFAULT` in every source-layer schema, shared and personal | `_SRC.ST_DEFAULT`, `DBT_USERNAME_SRC.ST_DEFAULT` |
...
Every engineer holds `RL_<PROJECT>_DEV__ENG` on the same `DB_<PROJECT>_DEV`. To keep people out of each other's way, `dev` does not use the `_<LAYER>` schemas; each engineer gets personal copies prefixed with `SNOWFLAKE_SCHEMA` (`DBT_<USERNAME>`, written by `just sf setup`). Terraform provisions them per user file (`terraform/personal.tf`; the prefix is the file's `schema_prefix`, else `DBT_` plus the login before the `@`); dlt and dbt cannot create schemas, so a schema that does not exist means the user file has not been applied yet. The engineer role holds the privileges, so engineers can technically read and write each other's personal schemas:
...
| Metadata | `DBT_USERNAME_MTD` | `_MTD` | dbt (`dbt_common` hook): `pre__dbt__*` run metadata, tables created on demand by the first real run |
...
| (no layer) | `DBT_USERNAME` | `_TMP` | dbt models without a `+schema` config (`target.schema`); not provisioned in `dev`, so such a model fails there |
...
| `upload_results` | `on-run-end` upload of run metadata into `pre__dbt__*` tables in the metadata layer (vendored `dbt_artifacts` upload machinery, Snowflake-only trim; `macros/dbt_artifacts/README.md` explains what was kept). Creates the tables if they do not exist; the schema (`_MTD`, or `<prefix>_MTD` in `dev`) is provisioned by Terraform. Skipped on the `dummy` target; `default_fallbacks.sql` provides `default__` variants so the DuckDB parse still resolves dispatch. |
