...
3. In `dev` every engineer works in personal schemas `<SNOWFLAKE_SCHEMA>_<LAYER>` (`DBT_INFO_STG`) of the shared development database; the other environments use the provisioned `_<LAYER>` schemas.
