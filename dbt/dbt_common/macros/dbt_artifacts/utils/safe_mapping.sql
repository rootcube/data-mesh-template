{% macro update_nested_dict(dictionary) %}
    ...
    {% for key, value in dictionary.items() %}
        {% if value is mapping %}
            ...
            {% set nested_update = dbt_common.update_nested_dict(dictionary[key]) %}
            {% do updated_dict.update({key: dbt_common.type_handler(nested_update)}) %}
        {% else %}
            {# Update the value dynamically #}
            {% do updated_dict.update({key: dbt_common.type_handler(value)}) %}
        {% endif %}
    {% endfor %}
    {{ return(updated_dict) }}
{% endmacro %}
...
{% macro safe_copy_mapping(dictionary) %}
    {{ dbt_common.raise_equality_warning(dictionary) }}
    {{ return(dbt_common.update_nested_dict(dictionary)) }}
{% endmacro %}
...
{% macro type_handler(val) %}
    ...
    {% if _val is none %}
        {{ return(_val) }}
    {% else %}
        {% if dbt_common.is_serializable(_val) %}
            {{ return(_val) }}
        {% else %}
            ...
            {% if _val.strftime is not none %}
                ...
                {{ return(_val.strftime(dbt_common.get_strftime_format()) ) }}
            {% else %}
                {#- just send it -#}
                {{ return(_val | as_text ) }}
            {% endif %}
...
{% macro raise_equality_warning(dictionary) %}
    ...
    {% if is_dev %}
        {% if dictionary != dbt_common.update_nested_dict(dictionary) %}
             {{ log("Caught on: " ~ dictionary.name, is_dev) }}
            {{ log("ORIGINAL !!! SEE BELOW !!!\n" ~ dictionary) }}
            {{ log("COPY !!! SEE BELOW !!!\n" ~ dbt_common.update_nested_dict(dictionary)) }}
